...
class StudentPaymentController extends Controller
{
    ...
    public function history()
    {
        ...
        $monthNow = now()->format('F Y');
        $price = Prices::where('level_id', $authUser->level_id)->where('class_id', $authUser->class_id)->first();
        $amountPaid = $price ? $price->price : 0;
        return view('students.payment.history', compact('payments', 'monthNow', 'amountPaid'));
    }
